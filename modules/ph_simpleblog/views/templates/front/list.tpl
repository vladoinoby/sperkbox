...
{if Configuration::get('PH_BLOG_DISPLAY_BREADCRUMBS')}
	{capture name=path}
		...
		{if isset($parent_category)}
			...
			<a href="{$link->getModuleLink('ph_simpleblog', 'category', ['sb_category' => $parent_category->link_rewrite])|escape:'html':'UTF-8'}"><span itemprop="title">{$parent_category->name|escape:'html':'UTF-8'}</span></a>
		{/if}
...
<div class="ph_simpleblog simpleblog-{if $is_category}category{else}home{/if}">
	...
	{if isset($posts) && count($posts)}
					...
					<div class="post-item">
						...
						{else}
							...
							<div class="post-thumbnail">
								<a href="{$post.url|escape:'html':'UTF-8'}" title="{l s='Permalink to' mod='ph_simpleblog'} {$post.title|escape:'html':'UTF-8'}" itemprop="url">
									{if $blogLayout eq 'full'}
										<img src="{$post.banner_wide|escape:'html':'UTF-8'}" alt="{$post.title|escape:'html':'UTF-8'}" class="img-responsive"  itemprop="image"  />
									{else}
										<img src="{$post.banner_thumb|escape:'html':'UTF-8'}" alt="{$post.title|escape:'html':'UTF-8'}" class="img-responsive"  itemprop="image" />
									{/if}
								</a>
							</div><!-- .post-thumbnail -->
							{/if}
							...
							<div class="post-content" itemprop="text">
								...
								<div class="post-read-more">
									<a href="{$post.url|escape:'html':'UTF-8'}" title="{l s='Read more' mod='ph_simpleblog'}">
										{l s='Read more' mod='ph_simpleblog'} <i class="icon icon-chevron-right"></i>
									</a>
								</div><!-- .post-read-more -->
								{/if}
							</div><!-- .post-content -->	
						...
						{else}
							<div class="post-additional-info post-meta-info">
								...
								{if $is_category eq iconlse && Configuration::get('PH_BLOG_DISPLAY_CATEGORY')}
									<span class="post-category">
										<i class="icon icon-tags"></i> <a href="{$post.category_url}" title="{$post.category|escape:'html':'UTF-8'}" rel="category">{$post.category|escape:'html':'UTF-8'}</a>
									</span>
								{/if}
		...
		{else}
			{include file="./pagination.tpl" rewrite=iconlse type=iconlse}
		{/if}
	{else}
