	...
	{foreach from=$multipleAddresses item=address name=myLoop}
    	<div class="address col-xs-12 col-sm-6">
			<ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if}{if $smarty.foreach.myLoop.index % 2} alternate_item{else} item{/if} box">
                <li class="address_title"><h3>{$address.object.alias}</h3></li>
                {foreach from=$address.ordered name=adr_loop item=pattern}
                    {assign var=addressKey value=" "|explode:$pattern}
                    <li><!--<i class="fa fa fa-check-square-o"></i>-->
                ...
                <a class="btn btn-default button exclusive-medium2" style="color: white; margin-bottom: 5px;" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}" title="{l s='Update'}"><span><i class="fa fa-pencil"></i>{l s='Update'}</span></a>
                <a class="btn btn-default button exclusive-medium2" style="color: white; margin-bottom: 5px;" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}"><span><i class="fa fa-times-circle"></i>{l s='Delete'}</span></a></li>
            </ul>
        </div>
...
<div class="clearfix main-page-indent">
	<a href="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" title="{l s='Add an address'}" class="btn btn-default button button-medium"><span>{l s='Add a new address'}</span></a>
	<br />
</div>
