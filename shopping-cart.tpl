...
{else}
	...
	<div id="order-detail-content" class="table_block table-responsive">
		<table id="cart_summary" class="table2 {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if} table-responsive">
			<!--
			<thead>
				<tr>
					<th class="cart_product first_item text-center">{l s='Product'}</th>
				...
				<tbody>
					...
					{/if}
						<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
							...
							<td class="cart_discount_price text-center" data-title="{$discount.name}">
