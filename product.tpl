...
{if $errors|@count == 0}
	...
	{if $content_only}<a href="{$link->getProductLink($product)|escape:'html':'UTF-8'}" target="_parent" class="btn btn-default button button-medium quickview-full-btn"><span>
							{l s='Go to full product page'} <i class="icon-chevron-right right"></i>
						</span></a>{/if}
...
<!--
			...
			{if !$content_only}<a href="{$link->getmanufacturerLink($product->id_manufacturer, $product_manufacturer->link_rewrite)}" title="{l s='All products of this manufacturer'}"> {/if}  <span itemprop="brand" style="display: none">{$product_manufacturer->name}</span>
				...
				{if !$content_only}<a class="manname" href="{$link->getmanufacturerLink($product->id_manufacturer, $product_manufacturer->link_rewrite)}" title="{l s='All products of this manufacturer'}">{/if}<span itemprop="brand">{$product_manufacturer->name}</span>{if !$content_only}</a>{/if}{/if}
        {/if}
...
-->
	...
	{if !$product->is_virtual && $product->condition}
		...
		<!-- left infos-->
		<div class="pb-left-column col-xs-12 {if isset($warehouse_vars.product_right_block) && $warehouse_vars.product_right_block && !$content_only}show-right-info col-md-{$warehouse_vars.product_left_size} col-lg-{$warehouse_vars.product_left_size}{else}col-md-{$warehouse_vars.product_left_size} col-lg-{$warehouse_vars.product_left_size}{/if} {if isset($warehouse_vars.thumbs_position ) && $warehouse_vars.thumbs_position}left-thumbs{/if}">
			<!-- product img-->
			...
			<div id="image-block" class="clearfix">
				...
				{if $have_image}
					...
					<button type="button" data-role="next" class="slick-next slick-arrow" aria-label="{l s='Next'}" role="button" style="display: block;">{l s='Next'}</button>
					<button type="button" data-role="prev" class="slick-prev slick-arrow" aria-label="{l s='Previous'}" role="button" style="display: block;">{l s='Previous'}</button>
					{/if}
					...
					{if !$content_only}
							...
							<i class="icon icon-arrows-alt"></i> <span>{l s='Expand'}</span>
							</span>
						{/if}
				{else}
						...
						{if isset($images)}
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}">
									<a{if $jqZoomEnabled && $have_image} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}" {if $image.id_image == $cover.id_image}class=" shown"{/if} {else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
										<img class="img-responsive" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'small_default')|escape:'html':'UTF-8'}" alt="{$imageTitle}" title="{$imageTitle}"  itemprop="image" 	{if isset($warehouse_vars.small_default)} height="{$warehouse_vars.small_default.height}" width="{$warehouse_vars.small_default.width}"{/if} />
									</a>
								</li>
							{/foreach}
...
<!--
            ...
            {if isset($product->wholesale_price) && $product->wholesale_price>0 }
            		<div class="feature_line" style="overflow: hidden;"><span class="feature_line_left"><b>{l s='Cena za GRAM:'}</b></span><span class="feature_line_right"><span itemprop="brand" itemscope="" itemtype="http://schema.org/Organization"> <span itemprop="name"><b>
            		{($product->wholesale_price*1.2)|string_format:"%.2f"} €
...
<!--
					{if !$PS_CATALOG_MODE}
						...
						{/if}
							<div class="add_to_cart_container {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} unvisible{/if}">
...
{/if}
{if $product->upc}
		<div style="overflow: hidden;"><span class="feature_line_left"><b><a href="https://argento.sk/vyhladavanie?orderby=position&orderway=desc&search_query={$product->upc|escape:'htmlall':'UTF-8'}">{l s='Kód pre súpravu:'}</a></b></span><span class="feature_line_right"><span itemprop="brand" itemscope="" itemtype="http://schema.org/Organization"> <span itemprop="name"><b><a href="https://argento.sk/vyhladavanie?orderby=position&orderway=desc&search_query={$product->upc|escape:'htmlall':'UTF-8'}">{$product->upc|escape:'htmlall':'UTF-8'}</a></b></span></span></span></div>
{/if}
