...
{if isset($products) && $products}
	...
	{foreach from=$products item=product name=products}
        ...
        <div style="display:block; text-align: center; margin-bottom: 10px;">
            {foreach from=$product.combinations.values key=id_product_attribute item=combination name=acccess}
							{if $smarty.foreach.acccess.index == 10}
    						{break}
  						{/if}
            ...
            {/foreach}
						<span style="padding: 2px; border: none 1px; color: #979797;"><a href="{$product.link|escape:'html'}" title="{$product.name|escape:html:'UTF-8'}" style="color: #979797;">&nbsp;</a></span>
        </div>
				{else}
				<div style="display:block; text-align: center; margin-bottom: 10px;">
						<span style="padding: 2px; margin-right: 2px; border: solid 1px; color: #979797; border-radius: 15px;"><a href="{$product.link|escape:'html'}" title="{$product.name|escape:html:'UTF-8'}" style="color: #979797;">3232</a></span>
        </div>
        ...
        {* Added for attributes *}
			...
			{if $id_default_group==4 or $id_default_group==6 or $id_default_group==9}
          <div class="product-descc" >
						{*$product.description_short|strip_tags:'UTF-8'|truncate:360:'...'*}
          <span class="product-reference">{if isset($product.reference)}Kód: {$product.reference}{else}&nbsp;{/if}
					{if isset($product.ean13) && $product.ean13!=="" && $id_default_group==4}
						JKPOV: {$product.ean13}<br />
					{/if}
					</span>
					{if $id_default_group==4}
						Dostupné množstvo: {$product.quantity_all_versions}<br />
					{/if}
					{if $id_default_group==4}
						{foreach from=$product.features item=feature}
							{if $feature.name=="Povrchová úprava"}
								Úprava: {$feature.value}
							{/if}
						{/foreach}
					{/if}
					...
					<div  itemscope class="content_price">
						{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
							<span  class="price product-price">
								...
								{if $id_default_group==4 or $id_default_group==6}
									VOC
								{/if}
					...
					<div  itemscope class="content_price">
						{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
							<span  class="price product-price">
						       ...
						       {convertPrice price=$product.wholesale_price*1.2}/g
								{/if}
								{if $id_default_group==3}
						       {convertPrice price=$product.wholesale_price*1.2}/g
								{/if}
								{if $id_default_group==4}
						      VOC {convertPrice price=$product.wholesale_price*0.5}/g
								{/if}
								{if $id_default_group==6}
						       VOC {convertPrice price=$product.wholesale_price*0.5}/g
								{/if}
								{if $id_default_group==9}
									 {convertPrice price=$product.wholesale_price*1.2}/g
								{/if}
